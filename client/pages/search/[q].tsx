    ...
    <Panel title="Search Results">
      ...
      {users &&
        users.map((user) => {
          return (
            <div>
              <Link href={`/profile/${user.aID}`}>
                {user.vchFirstName} {user.vchLastName}
              </Link>
            </div>
          );
        })}
      ...
      {tracks &&
        tracks.map((track) => {
          return (
            <div>
              <Link href={`/profile/${track.nArtistID}`}>
                {track.vchTrackName}
              </Link>
            </div>
          );
      ...
      {stores &&
        stores.map((store) => {
          return (
            <div>
              <Link href={`/profile/${store.nUserID}`}>
                {store.vchStoreName}
              </Link>
            </div>
          );
      ...
      {products &&
        products.map((product) => {
          return <div>{product.vchProductName}</div>;
        })}
    </Panel>
  );
}
